...
void FitDst()
{
    ...
    TH1D* hdat = new TH1D("hdat","D^{*+} #rightarrow D^{+}#pi^{0}, D^{+} #rightarrow K_{S}#pi^{-}#pi^{+}#pi^{+}",Nbins,lend,rend);
    TH1D* hsb = new TH1D("hsb","D^{*+} #rightarrow D^{+}#pi^{0}, D^{+} #rightarrow K_{S}#pi^{-}#pi^{+}#pi^{+}",Nbins,lend,rend);
    
    
    
    double Ntot, Nsig, dNsig, Nbkg, dNbkg;
    TCut Mwindow = Form("tag==3 && dstch==2 && dch==3 && abs(mks-0.4979)<0.0075 && mdst > %lf && mdst < %lf && abs(md-1.86965)<0.015",lend,rend);
