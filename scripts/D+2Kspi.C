{
    ...
    TH1D* hdat = new TH1D("hdat","D^{+} #rightarrow K_{S}#pi^{+}",Nbins,lend,rend);
    TH1D* hsb = new TH1D("hsb","D^{+} #rightarrow K_{S}#pi^{+}",Nbins,lend,rend);
    double hwidth = rend-lend, binw = hwidth/Nbins;
    
    
    double Ntot, Nsig, dNsig, Nbkg, dNbkg;
    TCut Mwindow = Form("tag==3 && dstch==2 && dch==2 && abs(mks-0.4979)<0.0075 && md > %lf && md < %lf",lend,rend);
    ...
    hdat -> GetXaxis()-> SetTitle("M(K_{S}#pi^{+}) [GeV]");
   ...
   // tstatfit -> DrawLatex(0.67, 0.53, Form("#sigma_{sig} = %0.4lf #pm %0.4lf",par[2], fdat -> GetParError(2)));
    ...
    tstatfit -> DrawLatex(0.67, 0.65, "D^{+} #rightarrow K_{S}#pi^{+}"); 
