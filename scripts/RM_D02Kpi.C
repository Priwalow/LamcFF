...
void RM()
{
    ...
    TH1D* hdat = new TH1D("hdat","D^{0}#rightarrow K^{-}#pi^{+}",Nbins,lend,rend);
    TH1D* hsb = new TH1D("hsb","temp",Nbins,lend,rend);
    double hwidth = rend-lend, binw = hwidth/Nbins;
    
    double D_mean = 1.8651, D_sigma = 0.0064;
    TCut D_window = Form("abs(md-%lf)<%lf",D_mean,3*D_sigma);
    TCut D_sb = Form("abs(md-%lf)>%lf && abs(md-%lf)<%lf",D_mean,4*D_sigma,D_mean,7*D_sigma);
    
    
    
    double Ntot, Nsig, dNsig, Nbkg3s, dNbkg3s;
    TCut Mwindow = Form("rmx > %lf && rmx < %lf",lend,rend);
    Ntot = ch1dat -> Draw("rmx>>+hdat","tag==1 && dch==1"+D_window+Mwindow,"goff"); //"lcch == 1 && ml>1.1 && ml<1.12"
    ch1dat -> Draw("rmx>>+hsb","tag==1 && dch==1"+D_sb+Mwindow,"goff");
    ...
    hdat -> GetXaxis()-> SetTitle("M_{recoil}(pD^{0}) [GeV]");
    ...
    */
    ...
    //leg->AddEntry("hsb","D0 sideband","l");
    leg -> SetBorderSize(0);
