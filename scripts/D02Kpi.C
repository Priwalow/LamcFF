{
    ...
    TH1D* hsb = new TH1D("hsb","D^{0} #rightarrow K^{-}#pi^{+}",Nbins,lend,rend);
    double hwidth = rend-lend, binw = hwidth/Nbins;
    
    
    double Ntot, Nsig, dNsig, Nbkg, dNbkg;
    TCut Mwindow = Form("dstch==1 && dch==1 && md > %lf && md < %lf",lend,rend);
    ...
    hdat -> GetXaxis()-> SetTitle("M(K^{-}#pi^{+}) [GeV]");
   ...
   // tstatfit -> DrawLatex(0.67, 0.53, Form("#sigma_{sig} = %0.4lf #pm %0.4lf",par[2], fdat -> GetParError(2)));
    ...
    tstatfit -> DrawLatex(0.67, 0.65, "D^{0} #rightarrow K^{-}#pi^{+}"); 
