{
    ...
    double lend=0, rend=3.5, MLambdac=2.28646; //lend=2.21, rend=2.36
    int Nbins=70;
    ...
    TCut Mwindow = Form("pvis > %lf && pvis < %lf",lend,rend);
    ...
    Ntot +=  ch1dat -> Draw("pvis>>+hdat","abs(mlc-2.28646)<0.015"+ commonLcLpiCut+Mwindow,"goff");
    ch1dat -> Draw("pvis>>+hsb",rmxrsbCut+commonLcLpiCut+Mwindow,"goff");
    ch1dat -> Draw("pvis>>+hsb",rmxlsbCut+commonLcLpiCut+Mwindow,"goff");
    ...
    hdat -> GetXaxis()-> SetTitle("p(#Lambda#pi^{+}#bar{p}#bar{D}^{*0})||p(#Lambda#pi^{+}#bar{p}D^{*-}#pi^{+}) in cms [GeV]");
