...
void FitDst()
{
    ...
    TH1D* hdat = new TH1D("hdat","D^{*+} #rightarrow D^{+}#pi^{0}, D^{+} #rightarrow K^{-}K^{+}#pi^{+}",Nbins,lend,rend);
    TH1D* hsb = new TH1D("hsb","D^{*+} #rightarrow D^{+}#pi^{0}, D^{+} #rightarrow K^{-}K^{+}#pi^{+}",Nbins,lend,rend);
    
    
    
    double Ntot, Nsig, dNsig, Nbkg, dNbkg;
    TCut Mwindow = Form("tag==3 && dstch==2 && dch==4 && mdst > %lf && mdst < %lf && abs(md-1.86965)<0.015",lend,rend);
    ...
    double initpar[] = {10000,MDst_p,0.0005,2.0044,1675,25000,2};//-1500000};
    fdat -> SetParameters(initpar); 
    fdat -> SetParLimits(1,MDst_p-0.002,MDst_p+0.002);
    fdat -> SetParLimits(2,0.0001,0.002);
    fdat -> SetParLimits(0,0,1e6);
    fdat -> SetParLimits(3,2.003,2.01);
    fdat -> SetParLimits(6,0,20);
