double MDst0=2.00685, MD0 = 1.86483, lend=1.998, rend=2.014;
int Nbins=160;
...
void FitDst()
{
    ...
    TH1D* hdat = new TH1D("hdat","D^{*0} #rightarrow D^{0}#pi^{0}, D^{0} #rightarrow K^{-}#pi^{+}",Nbins,lend,rend);
    TH1D* hsb = new TH1D("hsb","D^{*0} #rightarrow D^{0}#pi^{0}, D^{0} #rightarrow K^{-}#pi^{+}",Nbins,lend,rend);
    
    
    
    double Ntot, Nsig, dNsig, Nbkg, dNbkg;
    TCut Mwindow = Form("(tag==4 && dstch==1 && abs(md-1.86483)<0.015 && (abs(mks-0.497611)<0.0075 || (dch!=3 && dch!=6))) && mdst > %lf && mdst < %lf && abs(md-1.86483)<0.015",lend,rend);
    ...
    double initpar[] = {13000,MDst0,0.0005,6000,MDst0,0.001,2,253,-21};//-1500000};
   ...
   // cout << "Useful width = " << solve(par[1]+10*par[5]-par[6],A,B,C)-par[1]-4*par[5]+par[6] << endl;
    ...
    tstatfit -> DrawLatex(0.67, 0.65, "D^{*0} #rightarrow D^{0}#pi^{0}"); 
    TLine* line2 = new TLine(MDst0-0.002,0,MDst0-0.002,top*0.95);
    TLine* line3 = new TLine(MDst0+0.002,0,MDst0+0.002,top*0.95);
