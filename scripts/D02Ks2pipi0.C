{
    ...
    TH1D* hdat = new TH1D("hdat","D^{0} #rightarrow K_{S}#pi^{-}#pi^{+}#pi^{0}",Nbins,lend,rend);
    TH1D* hsb = new TH1D("hsb","D^{0} #rightarrow K_{S}#pi^{-}#pi^{+}#pi^{0}",Nbins,lend,rend);
    double hwidth = rend-lend, binw = hwidth/Nbins;
    
    
    double Ntot, Nsig, dNsig, Nbkg, dNbkg;
    TCut Mwindow = Form("dstch==1 && dch==6 && abs(mks-0.4979)<0.0075 && md > %lf && md < %lf",lend,rend);
    ...
    fdat -> SetParameters(10000,MD0,0.008,2000,0,10); //100,,0.01,40,-300
    ...
    hdat -> GetXaxis()-> SetTitle("M(K_{S}#pi^{-}#pi^{+}#pi^{0}) [GeV]");
  ...
  //  tstatfit -> DrawLatex(0.67, 0.59, Form("Mean_{sig} = %0.4lf #pm %0.4lf",par[1], fdat -> GetParError(1)));
   // tstatfit -> DrawLatex(0.67, 0.53, Form("#sigma_{sig} = %0.4lf #pm %0.4lf",par[2], fdat -> GetParError(2)));
    ...
    tstatfit -> DrawLatex(0.67, 0.65, "D^{0} #rightarrow K_{S}#pi^{-}#pi^{+}#pi^{0}"); 
    TLine* line2 = new TLine(MD0-0.015,0,MD0-0.015,top*0.95);
    TLine* line3 = new TLine(MD0+0.015,0,MD0+0.015,top*0.95);
