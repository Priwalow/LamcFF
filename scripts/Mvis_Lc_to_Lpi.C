{
    ...
    double lend=7, rend=12, MLambdac=2.28646; //lend=2.21, rend=2.36
    int Nbins=100;
    ...
    TCut Mwindow = Form("mvis > %lf && mvis < %lf",lend,rend);
    ...
    Ntot +=  ch1dat -> Draw("mvis>>+hdat","abs(mlc-2.28646)<0.015"+ commonLcLpiCut+Mwindow,"goff");
    ch1dat -> Draw("mvis>>+hsb",rmxrsbCut+commonLcLpiCut+Mwindow,"goff");
    ch1dat -> Draw("mvis>>+hsb",rmxlsbCut+commonLcLpiCut+Mwindow,"goff");
    ...
    hdat -> GetXaxis()-> SetTitle("M(#Lambda#pi^{+}#bar{p}#bar{D}^{*0})||M(#Lambda#pi^{+}#bar{p}D^{*-}#pi^{+})[GeV]");
