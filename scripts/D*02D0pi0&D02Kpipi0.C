...
Double_t myfdat(Double_t* x, Double_t *par)
{
   ...
   if (xx>par[6])
       return binw*(par[0]*TMath::Gaus(xx,par[1],par[2],true)+par[3]*TMath::Gaus(xx,par[4],par[5],true))+sqrt(xx-par[6])*(par[7]+par[8]*d+par[9]*d*d)+par[10];
   else return par[10];
}
...
void FitDst()
{
    ...
    TH1D* hdat = new TH1D("hdat","D^{*0} #rightarrow D^{0}#pi^{0}, D^{0} #rightarrow K^{-}#pi^{+}#pi^{0}",Nbins,lend,rend);
    TH1D* hsb = new TH1D("hsb","D^{*0} #rightarrow D^{0}#pi^{0}, D^{0} #rightarrow K^{-}#pi^{+}#pi^{0}",Nbins,lend,rend);
    
    
    
    double Ntot, Nsig, dNsig, Nbkg, dNbkg;
    TCut Mwindow = Form("tag==4 && dstch==1 && dch==4 && mdst > %lf && mdst < %lf && abs(md-1.86483)<0.015",lend,rend);
    ...
    TF1* fdat = new TF1("fdat",myfdat,lend,rend,11);
    double initpar[] = {10000,MDst0,0.0005,10000,MDst0,0.001,1.98,1675,25000,100,2};//-1500000};
    ...
    fdat -> SetParLimits(5,0.001,0.0025);
    fdat -> SetParLimits(0,0,1e6);
    fdat -> SetParLimits(6,1.998,2.005);
    ...
    covFit.GetSub(6,10,covBackground);
    double * par;
