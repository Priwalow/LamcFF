{
    ...
    TCut Mwindow = Form("rmvis_no*fabs(rmvis_no) > %lf && rmvis_no*fabs(rmvis_no) < %lf",lend,rend);
    ...
    Ntot +=  ch1dat -> Draw("rmvis_no*fabs(rmvis_no)>>+hdat","abs(mlc-2.28646)<0.015" + commonLcLpiCut+Mwindow,"goff");
    ch1dat -> Draw("rmvis_no*fabs(rmvis_no)>>+hrsb",rmxrsbCut+commonLcLpiCut+Mwindow,"goff");
    ch1dat -> Draw("rmvis_no*fabs(rmvis_no)>>+hlsb",rmxlsbCut+commonLcLpiCut+Mwindow,"goff");
    ...
    fdat -> SetParameters(50,0,0.05,1); //100,MLambdac,0.01,40,-300
    fdat -> SetParLimits(1,-0.1,+0.1);
    fdat -> SetParLimits(0,0,1e5);
    fdat -> SetParLimits(3,0,10);
    ...
    hdat -> GetXaxis()-> SetTitle("M^{2}_{recoil}(#Lambda#pi^{+}#bar{p}#bar{D}^{*0})||M^{2}_{recoil}(#Lambda#pi^{+}#bar{p}D^{*-}#pi^{+})[GeV^{2}]");
   ...
   // leg->AddEntry("fsig","Signal","l");
    ...
    leg -> AddEntry("hrsb","M(#Lambda#pi^{+}#pi^{0}) right sb","l");
    leg -> AddEntry("hlsb","M(#Lambda#pi^{+}#pi^{0}) left sb","l");
    leg -> SetBorderSize(0);
