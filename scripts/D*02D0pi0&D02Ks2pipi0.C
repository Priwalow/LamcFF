...
Double_t mybkg(Double_t* x, Double_t *par)
{
   ...
   if (xx>par[0])
       return sqrt(xx-par[0])*(par[1]+par[2]*d)+par[3]; //+par[3]*d*d
   else return par[3];
}
...
Double_t myfdat(Double_t* x, Double_t *par)
{
   ...
   if (xx>par[6])
       return binw*(par[0]*TMath::Gaus(xx,par[1],par[2],true)+par[3]*TMath::Gaus(xx,par[4],par[5],true))+sqrt(xx-par[6])*(par[7]+par[8]*d)+par[9]; //+par[9]*d*d
   else return par[9];
}
...
void FitDst()
{
    ...
    TH1D* hdat = new TH1D("hdat","D^{*0} #rightarrow D^{0}#pi^{0}, D^{0} #rightarrow K_{S}#pi^{-}#pi^{+}#pi^{0}",Nbins,lend,rend);
    TH1D* hsb = new TH1D("hsb","D^{*0} #rightarrow D^{0}#pi^{0}, D^{0} #rightarrow K_{S}#pi^{-}#pi^{+}#pi^{0}",Nbins,lend,rend);
    
    
    
    double Ntot, Nsig, dNsig, Nbkg, dNbkg;
    TCut Mwindow = Form("tag==4 && dstch==1 && dch==6 && abs(mks-0.4979)<0.0075 && mdst > %lf && mdst < %lf && abs(md-1.86483)<0.015",lend,rend);
    ...
    double initpar[] = {10000,MDst0,0.0005,2000,MDst0,0.0012,1.9997,265,-10000,15};//-1500000};
