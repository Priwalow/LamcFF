{
    ...
    TH1D* hdat = new TH1D("hdat","D^{+} #rightarrow K^{-}#pi^{+}#pi^{+}",Nbins,lend,rend);
    TH1D* hsb = new TH1D("hsb","D^{+} #rightarrow K^{-}#pi^{+}#pi^{+}",Nbins,lend,rend);
    double hwidth = rend-lend, binw = hwidth/Nbins;
    
    
    double Ntot, Nsig, dNsig, Nbkg, dNbkg;
    TCut Mwindow = Form("tag==3 && dstch==2 && dch==1 && md > %lf && md < %lf",lend,rend);
    ...
    hdat -> GetXaxis()-> SetTitle("M(K^{-}#pi^{+}#pi^{+}) [GeV]");
