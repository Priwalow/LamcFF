{
    ...
    double lend=-0.01, rend=0.01, MLambdac=2.28646; //lend=2.21, rend=2.36
    int Nbins=40;
    ...
    hdat -> GetXaxis()-> SetTitle("M^{2}_{recoil}(#Lambda#pi^{+}#pi^{0}#bar{p}#bar{D}^{*0})||M^{2}_{recoil}(#Lambda#pi^{+}#pi^{0}#bar{p}D^{*-}#pi^{+})[GeV^{2}]");
    hdat -> GetXaxis()-> SetTitleSize(axisFontSize);
    hdat -> GetXaxis()-> SetLabelSize(axisFontSize);
    hdat -> GetXaxis()-> SetTitleOffset(1.1);
    hdat -> GetYaxis()-> SetTitle(Form("Events /  %.0f MeV^{2} ",binw*1000000));
