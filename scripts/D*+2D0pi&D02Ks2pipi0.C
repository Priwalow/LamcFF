...
void FitDst()
{
    ...
    TH1D* hdat = new TH1D("hdat","D^{*+} #rightarrow D^{0}#pi^{+}, D^{0} #rightarrow K_{S}#pi^{-}#pi^{+}#pi^{0}",Nbins,lend,rend);
    TH1D* hsb = new TH1D("hsb","D^{*+} #rightarrow D^{0}#pi^{+}, D^{0} #rightarrow K_{S}#pi^{-}#pi^{+}#pi^{0}",Nbins,lend,rend);
    
    
    
    double Ntot, Nsig, dNsig, Nbkg, dNbkg;
    TCut Mwindow = Form("tag==3 && dstch==1 && dch==6 && abs(mks-0.4979)<0.0075 && mdst > %lf && mdst < %lf && abs(md-1.86483)<0.015",lend,rend);
