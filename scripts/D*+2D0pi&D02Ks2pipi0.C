...
void FitDst()
{
    ...
    TH1D* hdat = new TH1D("hdat","D^{*} #rightarrow D^{0}#pi, D^{0} #rightarrow K_{S}#pi#pi#pi^{0}, K_{S} #rightarrow #pi^{+}#pi^{-}",Nbins,lend,rend);
    TH1D* hsb = new TH1D("hsb","D^{*} #rightarrow D^{0}#pi, D^{0} #rightarrow K_{S}#pi#pi#pi^{0}, K_{S} #rightarrow #pi^{+}#pi^{-}",Nbins,lend,rend);
    
    
    
    double Ntot, Nsig, dNsig, Nbkg, dNbkg;
    TCut Mwindow = Form("tag==3 && dch==6 && dstch==1 && mdst > %lf && mdst < %lf && abs(md-1.8645)<0.0237",lend,rend);
    ...
    double initpar[] = {6500,MDst_p,0.0005,2000,MDst_p,0.0012,2,265,-10000};//-1500000};
    ...
    hdat -> GetXaxis()-> SetTitle("M(K_{S}#pi#pi#pi#pi^{0})-M(K_{S}#pi#pi#pi^{0})+M^{PDG}_{D^{0}}[GeV]");
