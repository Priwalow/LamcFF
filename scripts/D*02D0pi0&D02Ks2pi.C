...
void FitDst()
{
    ...
    TH1D* hdat = new TH1D("hdat","D^{*0} #rightarrow D^{0}#pi^{0}, D^{0} #rightarrow K_{S}#pi^{-}#pi^{+}",Nbins,lend,rend);
    TH1D* hsb = new TH1D("hsb","D^{*0} #rightarrow D^{0}#pi^{0}, D^{0} #rightarrow K_{S}#pi^{-}#pi^{+}",Nbins,lend,rend);
    
    
    
    double Ntot, Nsig, dNsig, Nbkg, dNbkg;
    TCut Mwindow = Form("tag==4 && dstch==1 && dch==3 && abs(mks-0.4979)<0.0075 && mdst > %lf && mdst < %lf && abs(md-1.86483)<0.015",lend,rend);
    ...
    double initpar[] = {6500,MDst0,0.0005,2000,MDst0,0.0012,2,265,-3000,2};//-1500000};
    ...
    fdat -> SetParLimits(6,1.997,2.005);
