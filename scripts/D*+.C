double MDst_p=2.01026, lend=2.003, rend=2.016, MDp=1.86965;
int Nbins=130;
...
Double_t mybkg(Double_t* x, Double_t *par)
{
   ...
   if (xx>par[0])
       return sqrt(xx-par[0])*(par[1]+par[2]*d+par[3]*d*d)+par[4];
   else return par[4];
}
...
Double_t myfdat(Double_t* x, Double_t *par)
{
   ...
   if (xx>par[3])
       return binw*par[0]*TMath::Gaus(xx,par[1],par[2],true)+sqrt(xx-par[3])*(par[4]+par[5]*d+par[6]*d*d)+par[7];
   else return par[7];
}
...
void FitDst()
{
    ...
    TH1D* hdat = new TH1D("hdat","D^{*+} #rightarrow D^{+}#pi^{0}, D^{+} #rightarrow K^{-}#pi^{+}#pi^{+}",Nbins,lend,rend);
    TH1D* hsb = new TH1D("hsb","D^{*+} #rightarrow D^{+}#pi^{0}, D^{+} #rightarrow K^{-}}#pi^{+}#pi^{+}",Nbins,lend,rend);
    
    
    
    double Ntot, Nsig, dNsig, Nbkg, dNbkg;
    TCut Mwindow = Form("(tag==3 && ((dstch==1 && abs(md-1.86483)<0.015 && (abs(mks-0.497611)<0.0075 || (dch!=3 && dch!=6)))) || (dstch==2 && abs(md-1.86965)<0.015 && (abs(mks-0.497611)<0.0075 || (dch!=2 && dch!=3)))) && mdst > %lf && mdst < %lf",lend,rend);
    ...
    TF1* fbkg = new TF1("fbkg",mybkg,lend,rend,5);
    TF1* fdat = new TF1("fdat",myfdat,lend,rend,8);
    double initpar[] = {10000,MDst_p,0.0005,1.98,1675,25000,10,10};//-1500000};
    ...
    covFit.GetSub(3,7,covBackground);
    double * par;
    ...
    hdat -> GetXaxis()-> SetTitle("M(D#pi)[GeV]");
    ...
    hdat -> GetYaxis()-> SetTitleOffset(1.1);
   ...
   // cout << "Useful width = " << solve(par[1]+10*par[5]-par[6],A,B,C)-par[1]-4*par[5]+par[6] << endl;
    ...
    tstatfit -> DrawLatex(0.67, 0.65, "D^{*+} #rightarrow D^{+}#pi^{0}; D^{0}#pi^{+}"); 
    TLine* line2 = new TLine(MDst_p-0.002,0,MDst_p-0.002,top*0.95);
    TLine* line3 = new TLine(MDst_p+0.002,0,MDst_p+0.002,top*0.95);
