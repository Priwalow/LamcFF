{
    ...
    int Nbins=100;
    TCanvas *c1 = new TCanvas("c1","Electron neutrino invariant mass",740,600);
    ...
    Ntot = ch1dat -> Draw("rm*fabs(rm)>>hdat","lcch == 3 && m*m-10.58*10.58+2*10.58*p-rm*rm > -2 && m*m-10.58*10.58+2*10.58*p-rm*rm < 2 && p>0.1 && mlc<2.2"+Mwindow,"goff");  
	...
	*/
    ...
    cout << "N nu_e: " << (int) Nsig <<" +- " << (int) dNsig << endl;
    ...
    hdat -> GetXaxis()->SetTitle("RM^{2}(#nu_{e}), GeV^{2}");
